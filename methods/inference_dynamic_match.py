...
from methods.inference import get_embeddings
...
def main(args, agent):
    ...
    for _, row in list(test_samples.iterrows()):
        ...
        context_embedding = get_embeddings(agent, [[row.Query]])
        for exemplars in whole_exemplars:
            for start_idx in range(0, len(exemplars), batch_size):
                ...
                faiss_score, cand_indices = faiss_index.search(response_emb.cpu().numpy().astype(np.float32) +
                                                               context_embedding.cpu().numpy().astype(np.float32), 1)
            ...
            whole_candidate_embeddings.append(
                get_candidate_embeddings(agent, [exemplar["response"] for exemplar in exemplars]))
    ...
    if args.save_results_path:
        ...
        with open(str(Path(args.save_results_path) / f"dynamicq-{args.styles[0]}{args.max_num_exemplars[0]}.jsonl"), "w") as f:
