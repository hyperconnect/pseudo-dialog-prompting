...
def main(args, agent):
    ...
    for _, row in list(test_samples.iterrows()):
        ...
        for exemplar in curr_exemplars:
            prefix_context.append(f"- {exemplar['response']}")

        prefix_context = f"The below are quotes of {args.character_name} during conversation.\n" + "\n".join(prefix_context) + f"\nThe below are conversation between User and {args.character_name}.\n"
    ...
    if args.save_results_path:
        ...
        with open(str(Path(args.save_results_path) / f"utterancesonly-{args.styles[0]}{args.max_num_exemplars[0]}.jsonl"), "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use-reversed-scores", action="store_true")
